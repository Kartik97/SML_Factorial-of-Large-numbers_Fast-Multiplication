...
fun act([],[],diff) = []
	...
	| act([],y::B,diff) = 
		let val l = act([],B,0)
		in if diff=1 then (y-1)::l else y::l
		end
	| act (x::A,y::B,diff) =
...
*)
fun sub(A,B) =  if length(A) > length(B) then (0,act(List.rev(A),List.rev(B),0))
				else if length(A) < length(B) then (1,act(List.rev(B),List.rev(A),0)) else
				if (bigger(A,B)) then (0,List.rev(act(List.rev(A),List.rev(B),0))) else (1,List.rev(act(List.rev(B),List.rev(A),0)))
...
fun final (A,B,C,l) = 
	let
		...
		val (C,AB) = (C,List.rev(add(List.rev(A),List.rev(B),0)))
		val (s,v) = sub(C,AB);
		val p_p1= A @ pad (2 * z);
		val p_p3 = v @ pad z
		(*val (P1,P2) = make(p_p1,B)*)
		val added = List.rev(add(List.rev(p_p1),List.rev(B),0))
		(*val (F,S) = make(added,p_p3)*)
	in
		 List.rev(add(List.rev(added),List.rev(p_p3),0))
	end
...
fun multiply (A,B,l) = 
	if(l=0) then []
		 ...
		 else let val (xl,xr) = List.splitAt(A,l div 2); val (yl,yr) = List.splitAt(B,l div 2)
	 		  ...
	 		  in let
		 	  		...
		 	  		val res1 = List.rev(add(List.rev(xl),List.rev(xr),0)); val res2 = List.rev(add(List.rev(yl),List.rev(yr),0)) ; 
...
fun fromString s = 
	...
	in let fun convert []=[]
				| convert (x::rest) = let val intlist = convert(rest) 
									  in if ord(x)>=48 andalso ord(x)<=57 then (ord(x)-48)::intlist else raise Invalid_Input_exception 
									  end
...
fun create ([],cur,inc)=[]
	| create (l,cur,inc) = 
		let
			val (s,cmp) = sub (l, cur)
		in
			if (trim(cmp) = [0] ) then [karatsuba l] else (karatsuba cur) :: create(l,List.rev(add(List.rev(cur),List.rev(inc),0)),inc)
		end
