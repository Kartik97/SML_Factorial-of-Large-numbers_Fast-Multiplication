...
*)
fun multiply (A,B) =
  let val la = length(A); val lb=length(B) 
  in  if (la=1 andalso lb=1) then singleMultiply(A,B)
      else if (lb <= ((la + 1) div 2)) then
          let val (b,a) = List.splitAt(A,(la div 2) + (la mod 2));
          in let
              val p1 = [0]; val p2 = multiply(b,B); 
              val res1 = add(a,b,0); val res2 = B ; 
            val p3=multiply(res1,res2);
            in
              final(p1,p2,p3,la)
            end
          end
      else if (la <= ((lb + 1) div 2)) then
           let val (d,c) = List.splitAt(B,(lb div 2) + (lb mod 2))
          in let
              val p1 = [0]; val p2 = multiply(A,d); 
              val res1 = A; val res2 = add(c,d,0) ; 
            val p3=multiply(res1,res2);
            in
              final(p1,p2,p3,lb)
            end
          end
      else let val (x,y) = (A,B);  
            val (b,a) = List.splitAt(x,(la div 2) + (la mod 2)); val (d,c) = List.splitAt(y,(lb div 2) + (lb mod 2))
          in let
              val p1 = multiply(a,c); val p2 = multiply(b,d); 
              val res1 = add(a,b,0); val res2 = add(c,d,0) ; 
            val p3=multiply(res1,res2);
            in
              if la>lb then final(p1,p2,p3,la) else final(p1,p2,p3,lb)
            end
          end
  end
...
fun create ([],cur)=[]
  | create (no,cur) = 
    ...
    in
      if (trim(cmp) = [0] ) then [karatsuba (List.rev(cur))] else (karatsuba (List.rev(cur))) :: create(no,add(cur,[1],0))
    end
